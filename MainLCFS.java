...
public class MainLCFS {
    ...
    //storing position
    static Stack<Process> readyq = new Stack<>();
...
//            readyq.push(p);  //set ready
    ...
    public static StringBuilder LCFS(){
...
//                            updateReady(runningP, true); //set ready bc no IO burst for this process
            ...
            for(int i =0; i< processSorted.size(); i++) {
                ...
                if(i==runningIndex) {
                    if (RunningToReady){
                        if(!readyq.isEmpty() && tempStack.isEmpty() && !runningOccupied){
                            offset = true;
                        }
                        if(readyq.isEmpty() && !tempStack.isEmpty() && !runningOccupied){
                            offset = true;
                ...
                if (current.isUnstarted()) {
                    if(T==current.arrival_time) {
                        if (runningOccupied) {
                            ...
                            if (tempStack.isEmpty() && readyq.isEmpty()) {       //temp & ready empty
                                updateRun(current, true);
                            }else {
                                //no one run, but either one is empty
                ...
                //ready
                    ...
                    // if exist in readyq, wait till tempq are done - have write off the value if in th
                    if( !runningOccupied) {
            ...
            //add all ready process to ready stack in a way that they can get popped in order



            //shift
            while(!tempStack.isEmpty()){
                readyq.push(tempStack.pop());
            }

            if(!readyq.isEmpty() && !runningOccupied){
                Process p = readyq.pop();
                updateRun(p, true);
                if(offset){
                    p.total_finish_time--;
                    p.total_wait_time--;
    ...
    public static StringBuilder PrintProcessData(){
        ...
        sb.append("The scheduling algorithm used was Last Come First Serve\n");
