...
//        }
    ...
    public static StringBuilder RR(){
        ...
        while(allDone!=0){
            ...
            int runningIndex =-1;
            int remain_quantumn = -1;
            boolean preempted = false;
            boolean ready = false;
            if(runningP!=null){

                remain_quantumn = runningP.quantumnLeft+1;
            ...
            for(int i =0; i< processSorted.size(); i++){
                ...
                if(i==runningIndex){
                    ...
                    sb.append(String.format("%12s %s.", "running", remain_quantumn));
                    continue;
                }
                ...
                if(current.isUnstarted()){
                    if(T == current.arrival_time){
                        if(readyq.isEmpty() && !runningOccupied){
                                updateRun(current,true);        //set to run
                        }else{
                            updateReady(current,true);      //someone run or someone is in front of you in readyq , set ready
                        }
                    }
                }

                else if(current.isReady()){
                    //top of queue no one runs
                    if(readyq.peek()==current && !runningOccupied){
                        updateRun(current,true);    //set run
                    }
                    else{
                        updateReady(current,false); //keep run
                    }
                }
                    ...
                    if(current.remain_current_io==0){
                        ...
                        updateBlocked(current,false);   //keep blocked
                    }
                }
    ...
    public static StringBuilder PrintProcessData(){
        ...
        sb.append("The scheduling algorithm used was Round Robbin\n\n");
